/**
 * @file main.c
 * @brief 校园通讯录管理系统 主程序入口 & 底层工具实现
 * @author 黄耀盟
 * @date 2026-06-27
 * @version 1.0
 * @description
 * 1. 定义全部全局同步句柄、全局数据结构、全局状态变量；
 * 2. 实现控制台基础工具：光标定位、隐藏光标、定位打印printL；
 * 3. UTF8字符宽度计算、弹窗提示、联系人新增/临时新增工具函数；
 * 4. 模糊查询For函数、文件持久化load读取/save保存实现；
 * 5. 三大核心线程：界面渲染draw、键盘输入getcin、窗口尺寸监听LW；
 * 6. main主函数：加载本地数据、创建事件/互斥锁、启动多线程、程序退出保存数据；
 * 7. 实现V4查询、V5全部展示、V6排序界面渲染，O4/O5/O6输入逻辑；
 * @cooperate 廖吉(T2.c删除模块)、李远豪(T3.c修改模块)
 * @compile MinGW/gcc编译，需链接Windows系统库
 */
